...
def import_data():
    ...
    try:
        ...
        # --- Daten einfügen mit manuellen IDs ---
        ...
        # Trailers (TrailerID pro Movie ab 1 hochzählen)
        for movie_id in range(1, 11):
        ...
        # Tickets – Seat als INT (1 bis AvailableSeats der Screening)
        ticket_id = 1
        cursor.execute("SELECT ScreeningID, AvailableSeats FROM Screening")
        screenings = cursor.fetchall()
        for screening in screenings:
            screening_id = screening['ScreeningID']
            max_seats = screening['AvailableSeats'] or 100  # Fallback falls NULL
            num_tickets = random.randint(0, min(40, max_seats))
            used_seats = set()
            for _ in range(num_tickets):
                # Zufälligen freien Seat wählen (keine Doppelbelegung)
                while True:
